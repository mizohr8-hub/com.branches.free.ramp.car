...
public class TutorialController : MonoBehaviour
{
    ...
    {
        Vector2 forwardScreenPosition = CameraControllerParkingJam.MainCamera.WorldToScreenPoint(steps[CurrentStepId].movable.ForwardPosition + Vector3.up);
        Vector2 backScreenPosition = CameraControllerParkingJam.MainCamera.WorldToScreenPoint(steps[CurrentStepId].movable.BackPosition + Vector3.up);

        Vector2 forwardPos = ScreenToCanvasPosition(forwardScreenPosition);
        Vector2 backPos = ScreenToCanvasPosition(backScreenPosition);
        ...
        while (true)
        {
            PointerImage.rectTransform.anchoredPosition = startPos;

            pointerCase = PointerImage.DOFade(1, 0.5f);
            yield return new WaitForSeconds(0.5f);

            pointerCase = PointerImage.rectTransform.DOAnchoredPosition(endPos, 1.5f).SetEasing(Ease.Type.SineInOut);
    ...
    private static Vector2 ScreenToCanvasPosition(Vector2 screenPosition)
    {
        Vector2 normalizedScreenPosition = new Vector2(screenPosition.x / CameraControllerParkingJam.MainCamera.pixelWidth, screenPosition.y / CameraControllerParkingJam.MainCamera.pixelHeight);

        return new Vector2(normalizedScreenPosition.x * TutorialCanvasRect.sizeDelta.x, normalizedScreenPosition.y * TutorialCanvasRect.sizeDelta.y);
    }
