...
public class ObstacleController : MonoBehaviour
{
    ...
    public bool npcObject = false;

    public bool IsTilting { get; private set; }
    public bool IsMovingNPC { get=>npcObject; }
    ...
    public bool ObstacleDestroyed { get => _destroyed; set => _destroyed=value; }
    ...
    public void InitNew()
    {
        if (isMoving && movementData.Length != 0)
        {
            StartCoroutine(Move());
        }
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (IsMovingNPC)
        {
            if (collision.transform.GetComponent<MovableController>())
            {                            
                Tween.Pause(TweenType.Update);                
            }
        }
    }
