...
public class MovableController : MonoBehaviour
{
    ...
    public FinishSnap finishSnap = new FinishSnap();

    private MaterialPropertyBlock propertyBlock;
    private List<Vector3> blockedDirections = new List<Vector3>();

    public GameObject[] expressionsAngry;
    public GameObject[] expressionsHappy;
    public bool isChallengeCar;

    public GameObject HammerPower;
    public GameObject MissilePower;
    public GameObject explosionParticle;
    public GameObject laneDestroyer;
    public GameObject targetSelection;

    public AudioClip driveAwayClip;

    public AudioSource SirenNoise;
    void Awake()
    {
    ...
    public void MissileAttackActivate()
    {
        UITouchHandler.Enabled = false;
        GameControllerParkingJam.IsMissileAttack = false;
        MissilePower.SetActive(true);
        LevelObjectsSpawner.TurnOffTargetSelectOfCars();
    }

    public void MissileDestroy()
    {
        movableRenderer.enabled = false;
        Instantiate(explosionParticle, transform.position, transform.rotation);
        Instantiate(laneDestroyer, transform.position, Quaternion.identity);
        UITouchHandler.Enabled = true;
        LevelController.MovableFinished(this);
    }
    public void HammerPowerActivate()
    {
        UITouchHandler.Enabled = false;
        GameControllerParkingJam.IsHammerAttack = false;
        LevelObjectsSpawner.TurnOffTargetSelectOfCars();
        HammerPower.SetActive(true);
    }

    public void LaneObjectDestroy()
    {
    ...
    public void Disable()
    {
        ...
        IsTilting = false;
        isCollisionMovement = false;
        for (int i = 0; i < expressionsHappy.Length; i++)
        {
            expressionsHappy[i].SetActive(false);
        }
        for (int i = 0; i < expressionsAngry.Length; i++)
        {
            expressionsAngry[i].SetActive(false);
        }
        StopAllCoroutines();
    }
    ...
    public void Init(MovableObjectData data) //car spawn
    {
        Data = data;
    ...
    private IEnumerator Shine()
    {
        ...
        Ease.EaseFunction ease = Ease.GetFunction(Ease.Type.SineOut);
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.layer == 9)
        {
            ...
            if (obstacle.IsMovingNPC && (isMoving || isMovingBackwards))
            {
                hitNPC = true;
            }
            ...
            for (int i = 0; i < expressionsAngry.Length; i++)
            {
                expressionsAngry[i].GetComponent<ParticleSystem>().Stop();
            }

            expressionsAngry[Random.Range(0, expressionsAngry.Length)].GetComponent<ParticleSystem>().Play();
        }
        ...
        if (hitNPC)
        {
            LevelController.LevelFail(0);
        }

        //Debug.LogError("Car collides with " + collision.gameObject.name);
    }
    ...
    private IEnumerator Collision(float xRotation, float zRotation)
    {
        ...
        CollisionData[] collisionData = GameControllerParkingJam.GameConfigurations.CollisionData;
        ...
        for (int i = 0; i < collisionData.Length; i++)
        {
            ...
            Ease.EaseFunction ease = Ease.GetFunction(collisionData[i].EaseFunction);
            ...
            do
            {
                ...
                float t = ease(time / collisionData[i].Duration);
    ...
    void OnTriggerEnter(Collider trigger)
    {
        ...
        if (driveAwayClip)
        {
            GameAudioController.PlayDrivingAwayAudioNew(driveAwayClip);
        }
        else
        {
    ...
    public IEnumerator MoveToFinish(Vector3 finishPosition)
    {
        ...
        expressionsHappy[Random.Range(0, expressionsHappy.Length)].GetComponent<ParticleSystem>().Play();
    ...
    public IEnumerator BeckwardsCircle()
    {
        ...
        float duration = this.speed / acceleration;
        ...
        do
        {
            ...
            float traversedDistance = this.speed * time - (acceleration * time * time) / 2;
