...
namespace RGSK
{
    ...
    public class MobileControlManager : MonoBehaviour
    {
        ...
        void Start()
        {
            playerControl = (GameObject.FindObjectOfType(typeof(PlayerControl))) ? GameObject.FindObjectOfType(typeof(PlayerControl)) as PlayerControl : null;
        ...
        public void SetMobileUiButtons()
        {
            ...
            if (!InputManager.instance || playerControl == null) return;
            ...
            foreach (UIButton b in allUIButtons)
            {
                ...
                if (b.buttonAction == UIButton.ButtonAction.SwitchCamera && playerControl.playerCam != null)
                    b.GetComponent<Button>().onClick.AddListener(() => playerControl.playerCam.SwitchCameras());
