...
namespace RGSK
{
    public class MenuManager : MonoBehaviour
    {
        ...
        public enum State { Main, VehicleSelect, myCustomise, TrackSelect, Customize, Settings, Loading }
        public State state;
        ...
        public GameObject myCustomizePanel;
        ...
        private AsyncOperation async,async2;
        ...
        void Awake()
        {
            ...
            instance = this;
            LoadValues();
            AnalyticsEvent("Main Menu");

            SecondMode();

            if (PlayerPrefs.GetInt("LevelNum") >= 4)
            {
                rampjumpUnlocked.SetActive(false);
            }
            if (PlayerPrefs.GetInt("LevelNum") >= 9)
            {
                multiplayerLocked.SetActive(false);
            }

            Screen.orientation = ScreenOrientation.Landscape;

            //PlayerData.currency += 100000;
            //FourthMode();
        }


        public GameObject locks,lock2;
        void SecondMode()
        {
            if (PlayerPrefs.GetInt("UnlockLevels")>=12)
            {
                locks.SetActive(false);
            }
        }
        public void PlayWheelShot()
        {
            loadingPanel.SetActive(true);
            StartCoroutine(LoadScene("MainMenu1"));
        }

        public void PlayParkingJam()
        {
            loadingPanel.SetActive(true);
            StartCoroutine(LoadScene("ParkingJamMainMenu"));
        }
        ...
        public void ShowRewrdVideo()
        {
            StartCoroutine(GiveReward());
        }

        public void SartOpenWorld()
        {
            loadingPanel.SetActive(true);
            PlayerPrefs.SetInt("PlayingOpenWorld", 1);
        ...
        IEnumerator GiveReward()
        {
            ...
            CarSelection.instance.GetReward();
        ...
        void Start()
        {
            ...
            //SceneManager.LoadScene("MyCarRaceSolo", LoadSceneMode.Single);
            //SceneManager.LoadScene("MyCarRaceAI", LoadSceneMode.Single);
            //async = SceneManager.LoadSceneAsync("MyCarRaceSolo1");
            //async.allowSceneActivation = false;
            //async2 = SceneManager.LoadSceneAsync("MyCarRaceAI");
            ...
            //hasnainpersuasive
            PersuasiveScript.instance.ShowUnlockAll();

            //consola
        }

        public GameObject multiplayerLocked;
        public GameObject[] RacingLevelLocks; 
        public GameObject rampjumpUnlocked;
        public void MultiplayerUnlocked()
        {
            if (PlayerPrefs.GetInt("LevelNum") >= 11)
            {
                multiplayerLocked.SetActive(false);
            }
        } 
        public void RampUnlocked()
        {
        ...
        void Update()
        {
            ...
            //print("solo mode Progress : "+async.progress);
        ...
        public void AnalyticsEvent(string msg)
        {
            //Firebase.Analytics.FirebaseAnalytics.LogEvent(msg);
            GameAnalyticsSDK.GameAnalytics.NewProgressionEvent(GameAnalyticsSDK.GAProgressionStatus.Start, msg);//HAiderGA
        }
        ...
        void UpdateUI()
        {
            ...
            if (playerCurrency) playerCurrency.text = PlayerData.currency.ToString();
            ...
            switch (state)
            {
                ...
                case State.myCustomise:
                    //mainPanel.SetActive(false);
                    print("Prited");
                    vehicleSelectPanel.SetActive(false);
                    myCustomizePanel.SetActive(true);
                    settingsPanel.SetActive(false);
                    loadingPanel.SetActive(false);
                    if (menuVehicles[vehicleIndex].charcter)
                    {
                        menuVehicles[vehicleIndex].charcter.gameObject.SetActive(false);
                    }
                    break;
                ...
                case State.TrackSelect:
                    ...
                    if (itemPrice) itemPrice.text = menuTracks[trackIndex].unlocked ? string.Empty : menuTracks[trackIndex].price.ToString("N0");
        ...
        public void UnlockRacingLevels()
        {
            for (int i = 0; i < PlayerPrefs.GetInt("RacingModeLevel") + 1; i++)
            {
                RacingLevelLocks[i].SetActive(false);
            }
        }
        ...
        public void Spoilders(int id)
        {
            for (int i = 0; i < menuVehicles[vehicleIndex].spilers.Length; i++)
            {
                menuVehicles[vehicleIndex].spilers[i].SetActive(false);
            }
            menuVehicles[vehicleIndex].spilers[id].SetActive(true);
        }
        ...
        public void PlayParkingMode()
        {
            loadingPanel.SetActive(true);
            StartCoroutine(LoadingScenes("Parking Mode"));
        }
        public void PlayOffroadMode()
        {
            loadingPanel.SetActive(true);
            StartCoroutine(LoadingScenes("OffRoad Mode"));
        }
        public void Play(string lvlName)
        {
            state = State.Loading;
            ...
            PlayerPrefs.SetInt("SelectedPlayer" , vehicleIndex);
            ...
            // PlayerPrefs.SetInt("Laps", menuTracks[trackIndex].laps);  //mjr
            //SceneManager.LoadScene(3);
            //StartCoroutine(LoadScene(lvlName));
            StartCoroutine(LoadingScenes(lvlName));
        ...
        public void PlayMulti()
        {
            ...
            PlayerPrefs.SetInt("Opponents", menuTracks[trackIndex].aiCount);
            // PlayerPrefs.SetInt("Laps", menuTracks[trackIndex].laps);  //mjr
            // SceneManager.LoadScene("MyCarRaceAI");
            if (OLDMulti)
            {
            StartCoroutine(LoadingScenes("MyCarRaceAI"));
            }
            else
            {
                StartCoroutine(LoadingScenes("RacingMode"));
                PlayerPrefs.SetInt("PlayingTournament", 1);
                PlayerPrefs.SetInt("TournamentStage", 0);
            }
        }
        public void EnterRacing(int LevelNumber)
        {
            PlayerPrefs.SetInt("RacingModeLevel", LevelNumber);
        ...
        public void CarID()
        {
            PlayerPrefs.SetInt("Carid", vehicleIndex);
            print("Vehicle id: " + PlayerPrefs.GetInt("Carid"));
        }


      public void LoadingScene()
        {
            StartCoroutine(LoadingScenes("HighwayRainy"));          
        }
        public void LoadingRampJump()
        {
            loadingPanel.SetActive(true);
            StartCoroutine(LoadingScenes("RampCarJump"));
        ...
        public void Buy()
        {
            ...
            if (state == State.VehicleSelect)
            {
                if (PlayerData.currency >= menuVehicles[vehicleIndex].price)
                {
                    ...
                    CreatePromptPanel(" ", "Do you really want to purchase this vehicle?");
                }
                else
                {
                    if (accept)
                    {
                     ...
                     // if (accept) accept.gameObject.SetActive(false);  //mjr
                    print("NO cash");
        ...
        void InvokDelay() 
        {
            mainMenu.instance.UnlockCarsBTn.SetActive(false);
        }
