...
namespace RGSK
{
    public class Statistics : MonoBehaviour
    {
        ...
        public void NewLap()
        {
            ...
            Debug.Log("Total Laps: " + RaceManager.instance.totalLaps);

            if (!infiniteLaps)
            {
                // Show final lap message
                if (lap == RaceManager.instance.totalLaps && RaceManager.instance.showRaceInfoMessages && gameObject.CompareTag("Player"))
                {
                ...
                {
                    Debug.LogWarning("Lap count reached or exceeded total laps. Ending race.");
                    lap = RaceManager.instance.totalLaps; // Ensures it doesn't go beyond
                    if (!knockedOut && !finishedRace)
                    {
                        FinishRace();
                        return; // Exit the function immediately after finishing the race
                    }
                }
            }

            // Reset lap timer if not a countdown timer
            if (RaceManager.instance.timerType != RaceManager.TimerType.CountDown)
            {
                lapTimeCounter = 0.0f;
            }

            // Handle knockout logic
        ...
        public void FinishRace()
        {
            ...
            {
                RaceManager.instance.EndRace(gameObject.GetComponent<Statistics>().rank);
            }
        ...
        void CheckForRespawn()
        {
            ...
            if (reviveTimer >= 5.0f)
            {
                RaceManager.instance.RespawnRacer(transform, lastPassedNode, 0.0f); //mjr
