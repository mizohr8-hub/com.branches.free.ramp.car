...
namespace RGSK
{
    public class RaceManager : MonoBehaviour
    {
        ...
        {
            int a = (PlayerPrefs.GetInt("Difficulty")+1);  //mjr
            totalRacers = a;
            print("total rAcers : " + a);
        }
        ...
        void Awake()
        {
            //create an instance
            instance = this;
            print("LoadRacePreference:  "  + loadRacePreferences);
        ...
        void SpawnRacers()
        {
            ...
            if (!playerCar)
            {
                ...
                {
                    playerCar = TournamentManager.Instance.PlayerPrefabs[PlayerPrefs.GetInt("SelectedPlayer")];
                }
                else
                {
                    playerCar = (GameObject)Resources.Load(ResourceFolder + PlayerPrefs.GetString("PlayerVehicle"));
                }
            ...
            while (startCountdown == true)
            {
                ...
                if (currentCountdownTime >= 1)
                {
                    if (countdownTimer < 0.01f)
                    {
                        ...
                        {
                            RaceUI.instance.countdown.SetActive(true);
                            RaceUI.instance.SetCountDownText(currentCountdownTime.ToString());

                            if (currentCountdownTime == 3) 
                            {
                                for (int i = 0; i < RaceUI.instance.starter.Length; i++)
                                {
                                    RaceUI.instance.starter[i].SetActive(false);
                                }
                                RaceUI.instance.starter[2].SetActive(true);
                            }
                            if (currentCountdownTime == 2) 
                            {
                                ...
                                RaceUI.instance.starter[1].SetActive(true);
                            }
                            if (currentCountdownTime == 1) 
                            {
                                for (int i = 0; i < RaceUI.instance.starter.Length; i++)
                                ...
                                RaceUI.instance.starter[0].SetActive(true);
                            }
                            //SoundManager.instance.PlayDefaultSound(SoundManager.instance.defaultSounds.countdownSoundNew[currentCountdownTime]);
                    ...
                    if (TournamentManager.Instance)
                    {
                        TournamentManager.Instance.RCCControllers.SetActive(true);
                    }
                    for (int i = 0; i < RaceUI.instance.starter.Length; i++)
                    {
                        RaceUI.instance.starter[i].SetActive(false);
                    }
                    RaceUI.instance.starter[3].SetActive(true);

                    //SoundManager.instance.PlayDefaultSound(SoundManager.instance.defaultSounds.startRaceSound);
                    //SoundManager.instance.PlayDefaultSound(SoundManager.instance.defaultSounds.countdownSoundNew[0]); Haider Comment
                    StartRace();

                    //Wait for 1 second and hide the text.
                    yield return new WaitForSeconds(1);
                    RaceUI.instance.countdown.SetActive(false);
        ...
        void Update()
        {
            ...
            if (isGrounded)
            {
                activePlayer.GetComponent<RCC_CarControllerV3>().enabled = true;
            }
            if (isGrounded)
            {
                activePlayer.GetComponent<RCC_CarControllerV3>().enabled = false;
            }
        ...
        {
            RCC_UIDashboardDisplay.Instance.controllerButtons.SetActive(true);//jj
        ...
        public void EndRace(int rank)
        {
            ...
            //Tournament Mode Rank Set
            if(TournamentManager.Instance)
            TournamentManager.Instance.SetRank(rank);
        ...
        public void PauseRace()
        {
            ...
            else
            {
                ...
                Time.timeScale = 1.0f;
                RaceUI.instance.racePanel.SetActive(true);
