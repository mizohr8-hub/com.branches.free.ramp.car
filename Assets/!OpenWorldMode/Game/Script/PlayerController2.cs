...
public class PlayerController2 : MonoBehaviour {
	...
	Rigidbody2D rb;
	public float speed, curveAfter, curveAmount;
	...
	IEnumerator CurvedForce()
	{
		for (int i = 0; i < curveOffsets.Length; i++)
			{
				yield return new WaitForSeconds(curveAfter);
				curveOffsets[i] = curveOffsets[i].normalized;
				rb.AddForce(curveOffsets[i] * speed*curveAmount);
