...
public class RCC_CarControllerV3 : MonoBehaviour {
	...
	internal float _steerInput
	{
		get
		{
			...
			{
				return steerInput + verticalAngle;//HighWayRacer
    ...
    private float NoSConsumption = 15;
    ...
    void CamFov()
    {
        cam = GameObject.FindGameObjectWithTag("MainCamera"); //mjr
        //fov = 70;// cam.GetComponent<Camera>().fieldOfView;  //mjr
    }
	...
	private void CheckBehavior (){
		...
		rigid.angularDrag = Mathf.Clamp(rigid.angularDrag, currentBehaviorType.angularDrag, 2f);//usman
		...
		//			highspeedsteerAngleAtspeed = Mathf.Clamp(highspeedsteerAngleAtspeed, 100f, maxspeed);
		//			antiRollFrontHorizontal = Mathf.Clamp(antiRollFrontHorizontal, 15000f, Mathf.Infinity);
		//			antiRollRearHorizontal = Mathf.Clamp(antiRollRearHorizontal, 15000f, Mathf.Infinity);
		//			gearShiftingDelay = Mathf.Clamp(gearShiftingDelay, 0f, .1f);
		//			rigid.angularDrag = Mathf.Clamp(rigid.angularDrag, .5f, 1f);
		//			break;
		//
...
//					engineTorqueCurve [i].SmoothTangents (2, .75f);
	...
	void CheckGameStatus()
	{
		...
		{
			if (speed <= 3 && GameHandler.instance.gameStarted)
			{
				//print("Thi sis called now n V3");
				GetComponent<TotalDistanceCalculator>().record = false;
    ...
    private void LateUpdate()
    {
		...
		{
			//NosFOV
			if (cam) cam.GetComponent<Camera>().fieldOfView = Mathf.Lerp(cam.GetComponent<Camera>().fieldOfView, 80, Time.deltaTime * 2);  //mjr
			...
			//neons
			//         for (int i = 0; i < GetComponent<BehaviourSetting>().neons.Length; i++)
			//         {
			//	GetComponent<BehaviourSetting>().neons[i].SetActive(true);
			//}
			if (GetComponent<BehaviourSetting>())
			{
				if (GetComponent<BehaviourSetting>().nos)
				{
					maxspeed = GetComponent<BehaviourSetting>().nosSpeed;
					GetComponent<BehaviourSetting>().nos.SetActive(true);
				}
			}
			
			if (GetComponent<NosSpeed>())
			{
				maxspeed = GetComponent<NosSpeed>().nosSpeed;
				GetComponent<NosSpeed>().nosParticles.SetActive(true);
				GameHandler.instance.nosParticle.SetActive(true);
				GetComponent<Rigidbody>().AddForce(transform.forward * 0.4f, ForceMode.VelocityChange);

				//transform.position = transform.position + new Vector3(-0.1f, 0, 0);
			}


			if (PlayerPrefs.GetInt("Modeis") == 0)
			{
				if (GamePlayManager1.instance)
				{
					GamePlayManager1.instance.nosImg.SetActive(true);
				}
			}
			else if (PlayerPrefs.GetInt("Modeis") == 1)
			{
				if (GamePlayManager1.instance)
				{
					GamePlayManager.instance.nosImg.SetActive(true);
				}
			}
		}
		//else if (GeneralScript.instance)
		//{
		//	if (GeneralScript.instance.slomo)
  //          {
		//		//maxspeed = 300f;
		//		if (cam)
		//		{
		//			cam.GetComponent<Camera>().fieldOfView = Mathf.Lerp(cam.GetComponent<Camera>().fieldOfView, 100, Time.deltaTime * 2);  //mjr
		...
		else
		{
			...
			if (cam)
			{
				cam.GetComponent<Camera>().fieldOfView = Mathf.Lerp(cam.GetComponent<Camera>().fieldOfView, 60, Time.deltaTime * 1);   //mjr
																																	   // print("No NOS");
				...
				if (PlayerPrefs.GetInt("Modeis") == 0)
				{
					if (GamePlayManager1.instance)
					{
						...
						GamePlayManager1.instance.nosImg.SetActive(false);
				...
				else if (PlayerPrefs.GetInt("Modeis") == 1)
				{
					...
					{
						GamePlayManager.instance.nosImg.SetActive(false);
				...
				//for (int i = 0; i < GetComponent<BehaviourSetting>().neons.Length; i++)
				//{
				//	GetComponent<BehaviourSetting>().neons[i].SetActive(false);
				//}
				if (GetComponent<BehaviourSetting>())
				{
					if (GetComponent<BehaviourSetting>().nos)
					{
						...
						maxspeed = GetComponent<BehaviourSetting>().maxSpeed;
						GetComponent<BehaviourSetting>().nos.SetActive(false);
					}
				}
				...
				{
					maxspeed = GetComponent<NosSpeed>().nosSpeed;
					GetComponent<NosSpeed>().nosParticles.SetActive(false);
					GameHandler.instance.nosParticle.SetActive(false);
        ...
        float a = (NoS / fullNos);
        float b = (speed / maxspeed);
        if (GamePlayManager.instance)
        {
           // GamePlayManager.instance.nosFillImage.fillAmount = a;
           // GamePlayManager1.instance.speedFillImage.fillAmount = b;
            //print(a + "Nos value");
        }
        if (GamePlayManager1.instance)
        {
            //GamePlayManager1.instance.nosFillImage.fillAmount = a*4;
            //GamePlayManager1.instance.speedFillImage.fillAmount = b;
            
        }
        CheckGameStatus();
        //Invoke("CheckGameStatus", 5f);


    }
    void Update (){

		if (!RearLeftWheelCollider.wheelCollider.isGrounded && !RearRightWheelCollider.wheelCollider.isGrounded && !FrontLeftWheelCollider.wheelCollider.isGrounded && !FrontRightWheelCollider.wheelCollider.isGrounded)
		{
			if (RGSK.RaceManager.instance)
			{
				print("ANgular Drag");
				//RGSK.RaceManager.instance.activePlayer.GetComponent<RCC_CarControllerV3>().canControl = false;
				GetComponent<Rigidbody>().angularDrag = 1.5f;
				GetComponent<Rigidbody>().drag = 0.02f;
				//GetComponent<Rigidbody>().drag = 1f;
			}
		else if (RGSK.RaceManager.instance)
			{
				//GetComponent<Rigidbody>().drag = 1f;
				//GetComponent<Rigidbody>().drag = 1f;
			}
    ...
    //        {
				//downForce = 20;
    //        }
			//GetComponent<BehaviourSetting>().inAir = true;
		}
		else
		{
			if (RGSK.RaceManager.instance)
			{
				print("ANgular Drag here");
			...
			//GetComponent<BehaviourSetting>().inAir = false;
		}
		if (canControl)
		{
...
//
    ...
    public void NOS(){
		...
		if(boostInput >= 0.08f  /*&& NoS > 0*/)  //mjr NoS > 5  boostInput >= 0.8  && _gasInput >= .8f
        {
           ...
           // print("NOS"); //mjr
           //kashif
            //BehaviourSetting.instance.aSource.enabled = true;
            //BehaviourSetting.instance.nosEffect.SetActive(true);

        }
        else{
            //kashif
            //if (BehaviourSetting.instance.nosEffect != null)
            //{
            //    BehaviourSetting.instance.nosEffect.SetActive(false);
            //    BehaviourSetting.instance.aSource.enabled = false;
            //}
            isNos = false;  //mjr
            ...
            NoSRegenerateTime += Time.fixedDeltaTime;  //mjr
